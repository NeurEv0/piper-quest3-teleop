...
<section><div class="section"><h2>Cleaning Ready</h2><div id="cleaningReady" class="status">CHECKING</div><div id="cleaningReasons" class="muted"></div></div><div class="section"><h2>系统预检</h2><div id="checks" class="checks"></div></div><div class="section"><h2>实时数据源</h2><div id="sources"></div></div></section>
</main><script>
...
async function resetEpisode(){try{const r=await request('/api/reset',{reason:'operator_reset'});message.textContent='Episode 已按 reset 边界中止并保留';setSignal('已重置边界：不可用于训练','failure',r.path||'')}catch(e){message.textContent=e.message;setSignal('重置失败','error',e.message)}}
...
function applyCaptureDefaults(s){if(defaultsApplied||!s.capture_defaults)return;operator.value=s.capture_defaults.operator_id||operator.value;task.value=s.capture_defaults.task_id||task.value;instruction.value=s.capture_defaults.language_instruction||instruction.value;defaultsApplied=true}
function render(s){applyCaptureDefaults(s);session.textContent=s.recorder.session_id||'';state.textContent=s.recorder.state;episode.textContent=s.recorder.episode_id||s.recorder.episode_path||'';cleaningReady.textContent=s.cleaning_ready?'READY':'BLOCKED';cleaningReady.className='status '+(s.cleaning_ready?'PASS':'BLOCKED');cleaningReasons.textContent=(s.cleaning_ready_reason_codes||[]).join(', ');const active=s.recorder.state==='RECORDING';start.disabled=active||!s.cleaning_ready;success.disabled=!active;failed.disabled=!active;abort.disabled=!active;addAction.disabled=!active;cameraOff.disabled=active;cameraMosaic.disabled=active;cameraOff.classList.toggle('selected',s.camera_mode==='off');cameraMosaic.classList.toggle('selected',s.camera_mode==='mosaic');if(!primitive.options.length&&s.vla_annotation)(s.vla_annotation.primitives||[]).forEach(v=>primitive.add(new Option(v,v)));if(s.recorder.state==='RECORDING')setSignal('正在录制','recording',s.recorder.episode_id||'');else if(s.recorder.state==='FINALIZING')setSignal('正在保存和校验','finalizing','请勿关闭进程');else if(s.recorder.error)setSignal('录制器错误','error',s.recorder.error);else if(s.last_outcome&&s.last_outcome.aborted)setSignal('最近 Episode 已中止','error',s.last_outcome.path||'');else if(s.last_validation){const marked=s.last_outcome&&s.last_outcome.task_success?'成功标记':'失败标记';setSignal(s.last_validation.valid?`已保存：${marked}`:'最近 Episode 校验失败',s.last_validation.valid?(s.last_outcome&&s.last_outcome.task_success?'success':'failure'):'error',(s.last_validation.errors||[]).join('; ')||(s.last_outcome&&s.last_outcome.path)||'')}else if(!s.cleaning_ready)setSignal('尚未就绪','error','请检查阻塞项');else setSignal('就绪，可以开始录制','success','');checks.innerHTML=s.preflight.checks.map(c=>`<div class="check"><strong class="${c.level}">${c.level}</strong><span>${c.name}: ${c.message}</span></div>`).join('');sources.innerHTML=Object.entries(s.sources||{}).map(([k,v])=>`<div class="metric"><span>${k}</span><span class="${v.level}">${v.message}</span></div>`).join('')}
async function poll(){try{const r=await fetch('/api/status');const s=await r.json();render(s);reset.disabled=s.recorder.state!=='RECORDING'}catch(e){state.textContent='DISCONNECTED'}setTimeout(poll,500)}poll();
</script></body></html>"""
...
class OperatorDashboard:
    def __init__(
        ...
        *,
        ...
        reset_episode: Callable[[dict[str, Any]], dict[str, Any]] | None = None,
    ):
        callbacks = {
        ...
        if reset_episode is not None:
            callbacks["/api/reset"] = reset_episode
