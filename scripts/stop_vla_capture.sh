...
if [[ -z "${MAIN_PIDS// }" ]]; then
  echo "Cannot identify the capture process-group leader; refusing an unsafe stop." >&2
  exit 1
fi
for pid in ${MAIN_PIDS}; do kill -TERM "${pid}" 2>/dev/null || true; done
...
if [[ -n "${REMAINING}" ]]; then
  echo "Safe shutdown timed out after 60s; terminating stale capture processes." >&2
