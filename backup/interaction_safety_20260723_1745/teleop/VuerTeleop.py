...
class VuerTeleop:
    ...
    def _mat_to_pose7(wrist_mat: np.ndarray) -> np.ndarray:
        ...
        wrist_mat = np.asarray(wrist_mat, dtype=float)
        if wrist_mat.shape != (4, 4) or not np.all(np.isfinite(wrist_mat)):
            raise ValueError("controller pose must be a finite 4x4 matrix")

        # WebXR matrices can accumulate a few parts-per-million of scale or
        # shear. Project the 3x3 block to the nearest proper rotation before
        # pytransform3d performs its strict SO(3) validation.
        u, _singular_values, vh = np.linalg.svd(wrist_mat[:3, :3])
        rotation = u @ vh
        if np.linalg.det(rotation) < 0.0:
            u[:, -1] *= -1.0
            rotation = u @ vh

        quat_wxyz = rotations.quaternion_from_matrix(rotation)
    ...
    def video_display_enabled(self) -> bool:
        return self.tv.video_display_enabled

    def set_video_display_enabled(self, enabled: bool) -> None:
        self.tv.set_video_display_enabled(enabled)
